...
export const ja = {
  ...
  // モード選択画面（開始前トップ）にだけ出す説明・注意点・関連記事。
  // 質問開始後はモード選択画面ごとアンマウントされるので自動的に出なくなる。
  top: {
    detailsToggle: 'このシミュレーターの詳しい説明を見る',
    whatYouLearnHeading: 'このシミュレーターで分かること',
    whatYouLearnItems: [
      '教育費が大きくなりやすい時期の目安',
      '住宅ローン完済までの見通しと、完済後の住居費の変化',
      '完全FIRE・サイドFIRE後の資産推移',
      '65歳以降の年金と老後生活費のバランス',
      '95歳時点の資産見通し',
      '長期の利回りを低めに見た「慎重条件」での変化',
      '条件を変えると改善の余地がある「見直しポイント」',
      '将来額と、今のお金の感覚で見た現在価値の違い',
    ],
    pvFutureHeading: '生活感覚で見る「現在価値」と、将来の額面で見る「将来額」',
    pvFutureBody:
      '現在価値は、将来のお金を「今のお金の感覚」で見た金額です。将来額は、その年に実際に表示される額面です。生活の実感や安心感を見るときは現在価値、将来の通帳残高や支払い額のイメージを見るときは将来額を中心にご覧ください。',
    cautionsHeading: 'ご利用前の注意点',
    relatedHeading: '次に読みたい関連記事',
    relatedBody:
      '今後、教育費・住宅ローン・サイドFIRE・老後資金などのテーマ別に、詳しい解説記事を追加していく予定です。',
  },
  common: {
    next: '次へ',
    back: '戻る',
  ...
  nav: {
    ...
    backToResult: '結果へ戻る',
    recompute: '再計算して結果へ',
    // 「続けて変更」: 再計算して結果に戻ったあと、「条件を変えてみる」セクションを開いて
    // 試行錯誤しやすくする補助導線。
    recomputeContinue: '続けて変更',
    // 未回答時の、止めない・煽らない案内
    ...
    // スマホ 375px でも 1 行に収まるよう短く。「未入力OK」で「未入力でも次へ進めます」の安心感を担う。
    stepStatus: (done: number, total: number): string =>
      `${done}/${total}入力済み${done < total ? '・未入力OK' : ''}`,
  ...
  result: {
    ...
    // What-if クイック調整（結果画面のその場再計算）
    quickAdjustHeading: '条件を少し動かして見る',
    quickAdjustNote: '押すたびにその場で再計算します。細かい変更は下の「条件を変えてみる」からできます。',
    // 前回比の差分チップ
    deltaPrefix: '前回の条件より',
